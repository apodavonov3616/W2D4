...
def sum_array(array)
    ...
    #if length of array is 1, first number is five, return first element
    #if two numbers, add last one to first one, and pop and call again
...
require "byebug"
def flatten(data)
    #so the base case is an array
    return [data] if data.is_a?(Array)==false
    #debugger
    #leaving a trail if things aren't an array
    #+= combines array, doesn't shovel
    flat = []
    data.each do |ele|
        flat += flatten(ele)
    end
